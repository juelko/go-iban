...
func (c Country) Generate(bban string) (*IBAN, error) {
	if !c.Pattern.MatchString(c.CountryCode.String() + "00" + bban) {
		return nil, ErrInvalidPattern
	}

	i := IBAN{cc: c.CountryCode}

	return i.generate(bban), nil
}

var (
	ErrNotCountryCode      = errors.New("Input should be ISO 3166-1 alpha-2 formatted country code")
	ErrCountryCodeNotFound = errors.New("Country code not found")
)
...
func NewCountryCode(cc string) (*CountryCode, error) {
	...
	return nil, ErrCountryCodeNotFound
}
